...
/**
 * Return a list of all logical volumes in the system.
 ...
 *
 * @param num_entries pointer to an integer to store the number of entries
 * @return list of FileListEntry elements containing all logical volumes
 */
    ...
    if (Examine(flock, &fileinfo)) {
        ...
        if (error != ERROR_NO_MORE_ENTRIES) {
            /* TODO: HANDLE THE IO ERROR
               We can't use puts() or text output in the library
             */
        }
    }
    UnLock(flock);
...
{
    if (!dirpath || strlen(dirpath) == 0) return all_volumes(num_entries);
