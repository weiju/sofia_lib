...
/* Directory name gadget data */
#define DIRSTRING_SIZE 80
#define DIRBUFFER_SIZE (DIRSTRING_SIZE + 2)
static UBYTE dirstr_buffer[DIRBUFFER_SIZE], dirstr_undo_buffer[DIRBUFFER_SIZE];
static struct StringInfo dirstr_info = {
    dirstr_buffer, dirstr_undo_buffer, 0, DIRSTRING_SIZE,
    0, 0, 0, 0, 0, 0,
    NULL, 0, NULL
};

/* File name gadget data */
#define FILESTRING_SIZE (40)
#define FILEBUFFER_SIZE (FILESTRING_SIZE + 2)
static UBYTE filestr_buffer[FILESTRING_SIZE], filestr_undo_buffer[FILESTRING_SIZE];
static struct StringInfo filestr_info = {
    filestr_buffer, filestr_undo_buffer, 0, FILESTRING_SIZE,
    0, 0, 0, 0, 0, 0,
    NULL, 0, NULL
};
...
static struct Gadget file_text = {&list_vslider, FILE_GADGET_X, FILE_GADGET_Y,
                                  ...
                                  &str_gadget_border, NULL, &labels[LABEL_FILE], 0,
                                  &filestr_info,
...
static struct Gadget dir_text = {&file_text, DRAWER_GADGET_X, DRAWER_GADGET_Y,
                                 ...
                                 GFLG_GADGHCOMP, GACT_RELVERIFY, GTYP_STRGADGET,
                                 &str_gadget_border,
                                 NULL, &labels[LABEL_DRAWER], 0,
                                 &dirstr_info,
                                 REQ_DIR_TEXT_ID, NULL};
...
static void update_input_gadget(struct Gadget *input_gadget, char *input_buffer, const char *str, int buffer_size)
{
    UWORD oldpos = RemoveGList(req_window, input_gadget, 1);
    strncpy(input_buffer, str, buffer_size);
    AddGList(req_window, input_gadget, oldpos, 1, &requester);
    RefreshGList(input_gadget, req_window, &requester, 1);
}
...
{
    if (entry->file_type == FILETYPE_DIR || entry->file_type == FILETYPE_VOLUME) {
        update_input_gadget(&dir_text, dirstr_buffer, entry->name, DIRBUFFER_SIZE);
    } else if (entry->file_type == FILETYPE_FILE) {
        update_input_gadget(&file_text, filestr_buffer, entry->name, FILEBUFFER_SIZE);
    }
}
 ...
 */
static void update_parent_button(const char *dirpath)
{
...
static void reload_file_list(const char *dirpath)
{
    ...
    update_parent_button(dirpath);
}

void show_logical_volume_list(void)
{
    // empty the directory and file input gadgets
    update_input_gadget(&dir_text, dirstr_buffer, "", DIRBUFFER_SIZE);
    update_input_gadget(&file_text, filestr_buffer, "", FILEBUFFER_SIZE);
    dir_buffer[0] = '\0';
    reload_file_list(NULL);
}

void handle_parent_button(void)
{
    // go to parent directory
    char *path_part = (char *) dc_PathPart(dir_buffer);
    *path_part = '\0';
 ...
 * Handle event loop.
 *
 * @return TRUE: user clicked Ok or double clicked on file entry
 *         FALSE: user canceled out
 */
        ...
        while (msg = (struct IntuiMessage *) GetMsg(req_window->UserPort)) {
            ...
            case IDCMP_MOUSEBUTTONS:
                if (msg->Code == SELECTUP) {
                    ...
                    if (entry != NULL && !entry->selected) {
                        ...
                        // Handle double click on file list entry
                        // It can either be a volume, a directory or a file
                        //printf("double click on: %s\n", entry->name);
                        if (entry->file_type == FILETYPE_VOLUME) {
                            // volume, so initialize the directory path buffer
                            // with that volume and reload the list with the
                            // elements in the root
                            strncpy(dir_buffer, entry->name, DIR_BUFFER_SIZE);
                            reload_file_list(dir_buffer);
                        } else if (entry->file_type == FILETYPE_DIR) {
                            // directory entry: add the directory to the
                            // current path in the directory buffer and
                            // reload the list with the entries in the new path
                            ...
                            dc_AddPart(dir_buffer, entry->name, DIR_BUFFER_SIZE);
                            //printf("DIR_BUFFER is NOW: '%s'\n", dir_buffer);
                            reload_file_list(dir_buffer);
                        } else if (entry->file_type == FILETYPE_FILE) {
                            // Confirm open file
                            close_requester();
                            dc_AddPart(dir_buffer, filestr_buffer, DIR_BUFFER_SIZE);
                            return TRUE;
                        }
                    }
                }
            ...
            case IDCMP_GADGETUP:
                ...
                case REQ_DRIVES_BUTTON_ID:
                    show_logical_volume_list();
                    break;
                case REQ_PARENT_BUTTON_ID:
                    handle_parent_button();
                    break;
                case LIST_UP_ID:
                    {
...
/**
 * Free memory for the list view bitmap.
 */
static void cleanup()
{
    for (int i = 0; i < filelist_bm_depth; i++) {
...
static BOOL open_request_window(struct Window *window, const char *title)
{
    BOOL retval = FALSE;
    InitRequester(&requester);
    newwin.Title = (char *) title;
    ...
    if (req_window = OpenWindow(&newwin)) {
        ...
        // clears the visual area of the list view
        clear_list();
...
const char *open_file(struct Window *window, const char *title, const char *confirm_text)
{
    BOOL status = FALSE;
    if (confirm_text) {
        labels[LABEL_OPEN].IText = (char *) confirm_text;
    }
    ...
    if (Request(&block_requester, window)) {
        ...
        status = open_request_window(window, title);
        EndRequest(&block_requester, window);
    } else {
        //printf("top request failed\n");
    }
    return status ? dir_buffer: NULL;
}
